...
$Suite = @(
  ...
  # doc-encoding-invariant (added 2026-09-07, issue #549) — the THIRD surface in the family
  # above, and until now the only one with nothing on it. journal-encoding-invariant guards
  # journals; ps1-encoding-sweep guards .ps1 source; the CATCH-UP DOC had no guard at all,
  # which is #549's title. Per #468 the doc is the primary communication mechanism, so
  # silent corruption there is a reliability defect, not a cosmetic one.
  # Mechanism is the same asymmetric pair described above, except STRICTLY WORSE: the doc
  # write is always UTF-8 (the Docs API), so there is no lossless half. EVERY ANSI read on
  # this path destroys. It cost task #471's first doc on 2026-09-05.
  # Reproduced on this box 2026-09-07 — and note the third line, which is why it hid:
  ...
  # The defect fires ONLY under the runtime the scheduled work actually uses, and is
  # invisible from the interactive shell — a check run in the convenient shell agrees with
  # the broken one.
  # DESTINATION-side by construction, which is #549's own note and it is correct: a lint of
  # the body being written would have passed on #471, because the file on disk was valid
  # UTF-8 — the corruption happened IN THE READ. It also cannot be fixed code-only, because
  # the doc write path is not a script: an agent issues mcp-probe calls ad hoc. There is no
  # single function to correct, so the check must be on the result.
  # Precision over recall, deliberately: every signature is a TWO-character sequence prose
  # does not produce. Measured against 53KB of real doc prose (#468 34,794 chars, #471
  # 18,794) dense with em-dashes, curly quotes and emoji — ZERO false positives; and against
  # mojibake generated by the real defect path — all 4 classes caught. A detector that cries
  # wolf on the primary surface gets switched off, and then the surface is unguarded while
  # appearing guarded, which is the exact shape this whole family exists to remove.
  # Bounded: 8 docs/run, 12h per-doc freshness (86 bound docs today), so it never becomes 86
  # API calls in one run (#586, #590). UNREADABLE is its own outcome, never folded into clean
  # (#346), and a CORRUPT doc is never stamped as checked, so a finding cannot age out of the
  # report while the document is still broken. 9 mutants killed by 9 arms in
  # mutcheck-doc-encoding-invariant.mjs, which also distinguishes a KILLED arm from a BROKEN
  # one — one arm was silently passing on a failed precondition until it did.
  # Exits 1 on findings; read 8 docs today, 0 corrupt.
  @{ n = 'doc-encoding-invariant'; bridge = $false }
