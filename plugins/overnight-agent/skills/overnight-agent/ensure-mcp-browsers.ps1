...
#   MCP Chrome 1        9222  %LOCALAPPDATA%\playwright-mcp\chrome1       (zoho + LastPass)
#   MCP Edge 1          9225  %LOCALAPPDATA%\playwright-mcp\edge1         (zoho + LastPass)
...
#   MCP Edge bijlanis   9228  %LOCALAPPDATA%\playwright-mcp\edge-bijlanis (bijlanis)
#
# USAGE:
#   .\ensure-mcp-browsers.ps1                 # start every slot whose port is down
#   .\ensure-mcp-browsers.ps1 -Slot edge1     # only that slot (chrome1 | edge1 | edge-bijlanis)
...
# profile (unlock LastPass + sign into Google/sites) must still be done by you
# inside that window. After that, cookies persist for every later launch.

param(
  [ValidateSet('chrome1','edge1','edge2','edge3','edge-bijlanis','all')]
  [string]$Slot = 'all',
  [switch]$Quiet
)
...
$slots = @(
  ...
  [pscustomobject]@{ Key='edge-bijlanis'; Exe=$edge;   Port=9228; Dir="$base\edge-bijlanis" }
)
