...
| `scripts/spec/readability.test.mjs` | `spec readability formatting`, `technical architecture document` | The system must wrap an exposed fenced example or module-path table in a collapsible, alert-coloured `<details>` block; must not nest or duplicate an existing compliant block, or let a second block borrow the first block's alert; and on `Technical-Architecture.md` must allow Mermaid diagrams while rejecting implementation code, and require that page to be linked from the main index. |
